...
Unity animation clips and controller in `Assets/_Game/Animations/Player` are project-authored integration data. Animation timing uses the creator's stated 20 FPS. Other characters, terrain, effects, attacks and additional movement animations from the pack are not imported.

## Kings and Pigs — Pig

- Creator: **Pixel Frog**, the same creator as the player's Virtual Guy.
